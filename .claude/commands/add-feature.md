...
**TDD（テスト駆動開発）の適用:**
CLAUDE.mdの「新機能は TDD 必須」に従い、各タスクは以下のサイクルで実装する：
1. テスト作成（Red）→ 2. 実装（Green）→ 3. リファクタリング（Refactor）
詳細は `/write-tests` コマンドまたは `@docs/testing-strategy.md` を参照。
...
-   ステップ 7: `make test`, `make lint`, `make typecheck`の全てのコマンドがエラーなく成功する。
